...
export default function SignInScreen() {
  ...
  async function handleSignIn() {
    ...
    try {
      ...
      Alert.alert(
        "Kirish xatosi",
        err?.errors?.[0]?.message ?? "Noma'lum xato"
      );
    } finally {
      setLoading(false);
    }
    ...
    <View style={[s.root, { paddingTop: insets.top + 20 }]}>
      ...
      >
        <View style={s.logoArea}>
          ...
          <Text style={s.tagline}>Sizning AI hamrohinggiz</Text>
        </View>
        ...
        <View style={s.footer}>
          <Text style={s.footerText}>Hisob yo'qmi? </Text>
          <Pressable onPress={() => router.push("/sign-up" as any)}>
            <Text style={s.footerLink}>Ro'yxatdan o'ting</Text>
          </Pressable>
        </View>
      </KeyboardAvoidingView>
    </View>
...
const s = StyleSheet.create({
  ...
  input: {
    flex: 1, color: FOREGROUND, fontSize: 15,
    paddingVertical: 14, fontFamily: "Inter_400Regular",
  },
  eyeBtn: { padding: 4 },
  btn: {
